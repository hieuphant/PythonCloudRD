...
from src.conf.env_setup import EnvSetup
...
class BasePage:
    ...
    def type_text(self, tuple_selector, value, timeout=EnvSetup.WAIT_TIMEOUT_SECONDS):
        elem = self.wait_for_visibility_of_element(tuple_selector, timeout)
        elem.send_keys(value)

    def get_element_text(self, tuple_selector, timeout=EnvSetup.WAIT_TIMEOUT_SECONDS):
        element = self.wait_for_visibility_of_element(tuple_selector, timeout)
        return element.text

    def click_element(self,tuple_selector, timeout=EnvSetup.WAIT_TIMEOUT_SECONDS):
        elem = self.wait_for_element_to_be_clickable(tuple_selector, timeout)
        elem.click()

    def wait_for_visibility_of_element(self, tuple_selector, timeout=EnvSetup.WAIT_TIMEOUT_SECONDS):
